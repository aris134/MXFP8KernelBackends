...
struct MXFlatmmArchTraits
{
    ...
    static auto preShuffleWeight(ck_tile::HostTensor<dtype>& src)
    {
        ...
        const int K                      = static_cast<int>(src_lengths[0]);
        const int N                      = static_cast<int>(src_lengths[1]);
        constexpr int packed_size        = ck_tile::numeric_traits<dtype>::PackedSize;
        const int KPack                  = std::is_same_v<dtype, ck_tile::pk_fp6x16_t>
                                               ? 32
                                               : 16 * packed_size;

        const int KLane = static_cast<int>(ck_tile::get_warp_size()) / static_cast<int>(NLane);
        ...
        for(int n = 0; n < N; ++n)
        {
            ...
            {
                const int n0 = n / static_cast<int>(NLane);
                const int n1 = n % static_cast<int>(NLane);
                ...
                const int outputIndex = n0 * KPack * static_cast<int>(NLane) * KLane * K0 +
                                        k0 * KPack * static_cast<int>(NLane) * KLane +
                                        k1 * KPack * static_cast<int>(NLane) + n1 * KPack + k2;
    ...
    static auto preShuffleScale(ck_tile::HostTensor<dtype>& src)
    {
        ...
        const std::size_t XdlMNThread   = Config::N_Warp_Tile;
        const std::size_t XdlKThread    = ck_tile::get_warp_size() / XdlMNThread;

        const auto MN_Padded = ck_tile::integer_least_multiple(MN, XdlMNThread * MNXdlPack);
        ck_tile::HostTensor<dtype> shuffled(ck_tile::HostTensorDescriptor({MN_Padded * K}, {1}));

        const std::size_t K0 = static_cast<std::size_t>(K) / KXdlPack / XdlKThread;

        for(std::size_t n = 0; n < static_cast<std::size_t>(MN_Padded); ++n)
        {
...
struct GroupedMXFlatmmKernel
{
    ...
    {
        return group_count * sizeof(FlatmmTransKernelArg<FlatmmScalePointer<-1>, FlatmmScalePointer<-1>, NumDTensor_>);
    }
    ...
    GridSize(const std::vector<GroupedMXFlatmmHostArgs<ScaleM, ScaleN, NumDTensor_>>& gemm_descs) -> dim3
    {
        index_t grid_size = 0;
    ...
    template <class ScaleM, class ScaleN>
    CK_TILE_HOST static auto
    MakeKargs(const std::vector<GroupedMXFlatmmHostArgs<ScaleM, ScaleN, NumDTensor_>>& gemm_descs)
    ...
    IsSupportedArgument(const std::vector<FlatmmTransKernelArg<ScaleM, ScaleN, NumDTensor_>>& kargs)
    {
    ...
    FindGroupId(const FlatmmTransKernelArg<FlatmmScalePointer<1, 32, e8m0_bexp_t>,
                                          FlatmmScalePointer<1, 32, e8m0_bexp_t>,
                                          NumDTensor_>* gemm_desc_ptr,
                index_t block_id,
                                   ...
                                   index_t group_count) const
    {
        ...
        const auto local_block_id = block_id - kargs.block_start;
        const auto block_idx_2d = OffsetTile1DPartitioner::GetOffsetedTileIndex(
            0,
            kargs.group_karg.M,
            kargs.group_karg.N,
            local_block_id % grid_size_2d);
                                   ...
                                   int n_repeat)
{
    ...
    if(Ks_host.empty())
        throw std::runtime_error("No active groups for grouped MX flatmm");

    const ck_tile::index_t K0 = Ks_host.front();
    for(std::size_t i = 1; i < Ks_host.size(); ++i)
        if(Ks_host[i] != K0)
            throw std::runtime_error("Grouped MX flatmm currently requires uniform K");
    ...
    return BaseFlatmmPipeline::template TailHandler<true>(
        [&](auto has_hot_loop_, auto tail_num_) {
            ...
            const std::size_t ws_bytes = kargs.size() * sizeof(typename decltype(kargs)::value_type);
            ck_tile::DeviceMem ws_dev(ws_bytes);
            ...
            const auto d_workspace_const =
                ck_tile::cast_pointer_to_constant_address_space(ws_dev.GetDeviceBuffer());
...
auto calculate_rtol_atol(const ck_tile::index_t K, const float max_accumulated_value)
{
    ...
    const auto atol =
        ck_tile::get_absolute_threshold<ComputeType, CDataType, AccDataType>(max_accumulated_value, K);
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if(K % ScaleGranularityK != 0)
            throw std::runtime_error("K must be divisible by 32 for MXFP8");
        if(E <= 0 || m_chunk <= 0)
        ...
        // Host reference / logical input tensors (unpadded logical layout)
        ...
        // Per-group packed/prepared device buffers.
        struct GroupMeta
        {
            ck_tile::index_t row_offset;
            ck_tile::index_t m_real;
            ck_tile::index_t m_pad;
        };
        std::vector<GroupMeta> metas;
        std::vector<std::unique_ptr<ck_tile::DeviceMem>> a_dev_bufs, b_dev_bufs, c_dev_bufs, a_scale_dev_bufs, b_scale_dev_bufs;
        ...
        for(ck_tile::index_t expert = 0; expert < E; ++expert)
        {
            ...
            auto b_shuffled       = MXTraits::preShuffleWeight(b_group);
            auto a_scale_shuffled = MXTraits::template preShuffleScale<true>(a_scale_group);
            auto b_scale_shuffled = MXTraits::template preShuffleScale<false>(b_scale_group);

            a_dev_bufs.push_back(std::make_unique<ck_tile::DeviceMem>(a_group.get_element_space_size_in_bytes()));
            b_dev_bufs.push_back(std::make_unique<ck_tile::DeviceMem>(b_shuffled.get_element_space_size_in_bytes()));
            c_dev_bufs.push_back(std::make_unique<ck_tile::DeviceMem>(m_pad * N * sizeof(CDataType)));
            a_scale_dev_bufs.push_back(std::make_unique<ck_tile::DeviceMem>(a_scale_shuffled.get_element_space_size_in_bytes()));
            b_scale_dev_bufs.push_back(std::make_unique<ck_tile::DeviceMem>(b_scale_shuffled.get_element_space_size_in_bytes()));

            a_dev_bufs.back()->ToDevice(a_group.data());
            b_dev_bufs.back()->ToDevice(b_shuffled.data());
            c_dev_bufs.back()->SetZero();
            a_scale_dev_bufs.back()->ToDevice(a_scale_shuffled.data());
            b_scale_dev_bufs.back()->ToDevice(b_scale_shuffled.data());
            ...
            b_ptrs_host.push_back(b_dev_bufs.back()->GetDeviceBuffer());
            ...
            scale_a_host.push_back(
                ck_tile::FlatmmScalePointer<ScaleGranularityM, ScaleGranularityK, ScaleType>{
                    static_cast<ScaleType*>(a_scale_dev_bufs.back()->GetDeviceBuffer()), m_pad});
            scale_b_host.push_back(
                ck_tile::FlatmmScalePointer<ScaleGranularityN, ScaleGranularityK, ScaleType>{
                    static_cast<ScaleType*>(b_scale_dev_bufs.back()->GetDeviceBuffer()), N});
        }

        // Timed region: grouped kernel only.
        const float ave_time = invoke_grouped_mx_flatmm_raw<MXTraits,
                                                            ADataType,
                                                            BDataType,
                                                            ck_tile::tuple<>,
                                                            AccDataType,
        ...
        const float tflops = static_cast<float>(flop) / 1.0e9f / ave_time;
        const float gbps   = static_cast<float>(num_byte) / 1.0e6f / ave_time;

        std::cout << "Run fp8 Grouped MX Flatmm kernel "
                  << " M_chunk = " << m_chunk
                  << " Experts = " << E
                  << " M_total = " << M_total
                  << " N = " << N
